...
# val_dataset = r'/projectnb/ec523/projects/Proj_road_segment_fix/EE8204-ResUNet/dataset/samples_val'
...
model = model_resunet.UNet((IMG_SIZE, IMG_SIZE, 3))
...
# Flatten dict of test data to NumPy arrays
X_test_flat = np.concatenate([X_test[k] for k in X_test], axis=0)
Y_test_flat = np.concatenate([Y_test[k] for k in Y_test], axis=0)

# Evaluate the model on the flattened test set
print("\nEvaluating on test set:")
results = model.evaluate(X_test_flat, Y_test_flat, verbose=1)
